...
技术报告、模型卡、系统卡只归公司动态；不占论文名额。自动论文入口限定正式 `journal-article`，同时检查主标题与副标题，排除可识别的综述、社论、概念评论、共识更新、命名规范、勘误、撤稿等。纯预印本暂不自动纳入。
...
- 每日 09:12 再进行一次备用更新，降低整点调度延迟或临时失败的影响；当天已有日报时保留原版。09:30 的每日巡检继续负责核对实际网页和必要的补发。
...
补充使用 Europe PMC 的近期与高引用摘要检索，覆盖脑、认知、人格评估和人类奖励学习等主题，发现候选后逐篇向 Crossref 核对 DOI、正式发表信息和引用数。观点、综述等类型不纳入候选，包括摘要中自称综述的文章；前两栏还检查摘要中是否包含人类行为、参与者、脑活动、人类数据集，或结合样本描述的自述与日记等研究证据。多来源合并时保留已获得的摘要和出处，避免空元数据覆盖有效资料。

论文检索覆盖语言模型与脑表征、人的决策、行为和人格等多个方向，合并后继续按固定栏目、引用和历史记录筛选。模型自身的“人格”研究不充当研究人的论文。Crossref 最多两个并行请求，遇到限流按 Retry-After 或退避间隔重试，最多四次。若 Crossref 与期刊页面没有可用摘要，则查询 Europe PMC 的公开摘要，并严格核对 DOI；无匹配原文时仍跳过该论文。

社会科学实验与人类共情研究也纳入检索。纯机器人社会模拟必须有真实人类数据的比较证据，才能进入前两栏；只用 LLM 撰写其他模型输出报告的研究，不归入“LLM 对人的理解与建模”。引用门槛和历史去重规则保持一致。
...
每份摘要生成后，再由模型对照同一份原文检查产品归属、指标对应关系、发言人和结论措辞。模型自查不能保证事实准确；人工核实过的修订存入 `data/summary_corrections.json`，按 DOI 或规范化 URL 匹配，在生成前优先使用相应修订，保留核对来源并标明人工修订。缺少原文的候选会被跳过。中文字段或格式不合格时，带明确纠错要求自动重试一次；单条公司动态仍失败则暂缓推送、显示数量提示，并留待后续重新筛选。合格论文不足三篇、论文摘要重试后仍失败或模型未能启动时，保留上一期。定时和手动运行都会保存诊断日志及已完成的摘要，保留7天。自动摘要仅基于公开材料，可能存在翻译或概括偏差，不能代替阅读全文。每个自动摘要保留来源 URL 与模型标识。
